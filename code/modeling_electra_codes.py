...
# CHANGE
from graph_learning import LatentGraph, WeightedGCNModel, GraphTransformerNet
import dgl
...
class ElectraAttention(nn.Module):
    def __init__(self, config):
        ...
        # CHANGE
        self.codes_attention = config.codes_attention
        self.only_codes = config.only_codes
        if self.codes_attention:
            # latent graph stuff
            self.graph_projection = nn.Linear(
                config.hidden_size, config.hidden_size)
            self.latent_graph = LatentGraph(config.threshold)
            # self.graph_model = GraphTransformerNet(config.hidden_size)
            self.graph_model = WeightedGCNModel(config.hidden_size)
    ...
    def forward(
        ...
        hidden_states,
        attention_mask=None,
        ## CHANGE: codes_attention_mask
        codes_attention_mask=None,
        head_mask=None,
        encoder_hidden_states=None,
        encoder_attention_mask=None,
        past_key_value=None,
        ...
        if self.codes_attention:
            """
            codes_self_outputs = self.codes_self(
                hidden_states,
                codes_attention_mask,
                head_mask,
                encoder_hidden_states,
                encoder_attention_mask,
                past_key_value,
            ...
            # Average the outputs of all_self and codes_self
            # combined = (self_outputs[0] + codes_self_outputs[0]) / 2
            # # # re-weigh the vectors by the codes attention
            # combined = combined.permute(0, 2, 1)
            # codes_attention = codes_self_outputs[1].squeeze()
            # output = torch.matmul(combined, codes_attention)
            # output = output.permute(0, 2, 1)

            # THIS SEEMS TO BE THE CORRECT WAY
            # we want: torch.matmul(attention, values), where attention is 512x512 and values is 512x256
            output = torch.matmul(
                codes_self_outputs[1].squeeze(), self_outputs[0])
            # output = torch.matmul(self_outputs[0].permute(0, 2, 1), codes_self_outputs[1].squeeze())
            # output = output.permute(0, 2, 1)
            output = output + self_outputs[0]

            attention_output = self.output(output, hidden_states)
            outputs = (attention_output,) + \
                self_outputs[1:] + codes_self_outputs[1:]
            """

            projections = self.graph_projection(hidden_states)
            projections = get_activation('gelu')(projections)
            pairwise_distances = torch.cdist(projections, projections)

            # only consider codes for efficiency
            if self.only_codes:
                mask_1 = codes_attention_mask.squeeze().unsqueeze(
                    1).expand(-1, pairwise_distances.shape[-1], -1)
                mask_1 = mask_1 * -1
                mask_2 = mask_1.permute(0, 2, 1)
                pairwise_distances = pairwise_distances + mask_1 + mask_2

            latent_graph = self.latent_graph(pairwise_distances)

            index = [lg.nonzero(as_tuple=False).t().contiguous()
                     for lg in latent_graph]
            values = [lg[idx[0], idx[1]]
                      for idx, lg in zip(index, latent_graph)]
            graphs = [dgl.graph((idx[0], idx[1])) for idx in index]
            for i, g in enumerate(graphs):
                if self.only_codes:
                    num_codes = (codes_attention_mask[i] == 0).sum().item()
                    g.ndata['h'] = projections[i][:num_codes]
                    g.edata['w'] = values[i]
                else:
                    g.ndata['h'] = projections[i]
                    g.edata['w'] = values[i]
            g = dgl.batch(graphs)
            ...
            gs = dgl.unbatch(g)
            if self.only_codes:
                graph_output = torch.stack([torch.cat([torch.zeros(
                    [hidden_states.shape[-2] - g.ndata['h'].shape[0], g.ndata['h'].shape[1]]).to(g.device), g.ndata['h']]) for g in gs])
            else:
                graph_output = torch.stack([g.ndata['h'] for g in gs])
            graph_output = (graph_output + self_outputs[0]) / 2
            attention_output = self.output(graph_output, hidden_states)
...
class ElectraModel(ElectraPreTrainedModel):
    def __init__(self, config):
        ...
        ## CHANGE ##
        # WE'RE GOING TO DO EMBEDDINGS_PROJECT TO HELP MAP CODES EMBEDDINGS TO THE SAME SPACE
        self.embeddings_project = nn.Linear(
            config.embedding_size, config.hidden_size)
        # self.LayerNorm = nn.LayerNorm(config.hidden_size, config.hidden_size)
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    def forward(
        ...
        if codes_attention_mask is None:
            codes_attention_mask = torch.ones(input_shape, device=device)
        if token_type_ids is None:
            token_type_ids = torch.zeros(
        ...
        # CHANGE: extend codes attention mask
        extended_codes_attention_mask = self.get_extended_attention_mask(
            codes_attention_mask, input_shape, device)
        ...
        hidden_states = self.encoder(
            ...
            # CHANGE: codes attention
            codes_attention_mask=extended_codes_attention_mask,
            head_mask=head_mask,
            output_attentions=output_attentions,
